...
A dual-sided price-level ladder, with Polymarket's real merge/redeem
mechanic as the only exit.

1. **Entry:** each tick, for **both UP and DOWN independently**, look
   at the current best ask and best bid. Price levels run **0.10 to
   0.95** in **0.01** steps (Polymarket's real tick size), and **each
   level trades at most once per side per window**:
   - if the best **ask** sits at a level not yet used, buy **10
     shares** there. This crosses the spread — an immediate taker
     fill, real taker fee applies.
   - if the best **bid** sits at a level not yet used, place a
     **resting** 10-share buy there instead (maker order). It only
     fills later, if the market's ask trades down to that price —
     earns the maker rebate instead of paying a fee.
...
comes from buying UP and DOWN cheaply enough, across enough levels,
that a merged pair costs less than the $1 it redeems for (e.g. UP at
0.20 + DOWN at 0.75 nets $0.05/pair before fees). It does **not**
depend on predicting which side wins. The real risks are (a) never
building a matched pair at all if the book gaps past levels instead of
trading through them, and (b) leftover one-sided inventory eating a
full loss at resolution if the two sides don't fill evenly. Validate
thoroughly in paper mode before ever pointing this at real money.
...
app/
  ...
  engine.py                the strategy: dual-sided price-level ladder, merge-as-exit, capital tracking
