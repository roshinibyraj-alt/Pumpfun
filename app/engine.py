...
current best bid. If it sits at a price level (config.ENGINE_LEVEL_STEP
increments, within [ENGINE_PRICE_MIN, ENGINE_PRICE_MAX]) not yet used
this window, place a resting buy for ENGINE_ORDER_SHARES there. This is
a pure maker order -- it never crosses the spread, and only fills on a
later tick if the market's ask trades down to (or through) that price.
Maker fills earn the rebate, never pay a fee.

MAKER-ONLY, DELIBERATELY: this engine never buys at the ask. In a real
two-sided market, ask_up + ask_down normally sits ABOVE $1.00 (that gap
is the spread/vig), so crossing the spread on both sides and merging
means paying more than $1 to redeem $1 -- a guaranteed loss, which is
exactly what an earlier ask-crossing version of this engine did and why
it consistently lost money. bid_up + bid_down normally sits BELOW
$1.00 -- the mirror image -- so filling only at the bid on both sides
and merging locks in that spread as profit instead of paying it away,
on top of the maker rebate on every fill.

Once a level (bid) has been targeted, it's marked used and never
targeted again this window -- exactly one entry per level, per side.
Resets every window since each 5-minute window is a brand-new
market/token with a fresh book.
...
This is a market-making strategy: profit comes from the bid-side spread
(bid_up + bid_down < $1) plus the maker rebate, not from predicting
which side wins. The real risks are (a) never accumulating a matched
pair at all if the book gaps past levels instead of trading down
through them (a resting bid can simply never fill), and (b) leftover
one-sided inventory eating a full loss at resolution if the two sides'
fills don't land in equal size. Validate thoroughly in paper mode.
"""
import time
...
class Engine:
    ...
    def reset_for_window(self, window: WindowMarket):
        ...
        self.broker.log_event(
            ...
            note=(f"maker-only ladder active on both sides, {config.ENGINE_PRICE_MIN}-{config.ENGINE_PRICE_MAX} "
                  f"@ {config.ENGINE_LEVEL_STEP} steps, {config.ENGINE_ORDER_SHARES:.0f}sh/order, "
                  f"resting bids only (no ask-crossing), one entry per level; merge = exit"),
            balance_after=self.s.balance,
        )
    ...
    def on_tick(self, up_bid: Optional[float], up_ask: Optional[float],
        ...
        self._check_new_entries(Side.UP, up_bid)
        self._check_new_entries(Side.DOWN, down_bid)
        self._try_merge()

    # ---- entries --------------------------------------------------------

    def _check_new_entries(self, side: Side, bid: Optional[float]):
        ...
        if not self.s.halted:
            ...
            if self.s.fills_this_window == 0:
                ...
                self.broker.log_event(
                    ...
                    balance_after=self.s.balance,
                    note="no fills this window -- neither side's bid ever reached the tradeable range, or no resting order got hit",
                )
